...
const DEBUG = JSON.parse(process.env.npm_config__evolution_chain_chain_debug || false);
...
class ChainStore
{
    ...
    */
    fetchObject( id, force = false )
    {
        ...
        if (id.search(delegate_prefix) === 0) this._subTo("delegates", id);
    ...
    */
    getDelegateById(account_id) {
        ...
        if (delegate_id === undefined) {
            ...
            this._subTo("delegates", delegate_id);
        }
    ...
    */
    ...
    //                 }
    //             }, error => {
    //                 let delegate_id = this.committee_by_account_id.set( id_or_account, null )
    //             })
    //         }
    //         else
    //         {
    ...
    fetchDelegateByAccount( account_id )
    {
        return new Promise( (resolve,reject ) => {
            ...
            .then( optional_delegate_object => {
                ...
                {
                    this._subTo("delegates", optional_delegate_object.id);
    ...
    _updateObject( object, notify_subscribers = false, emit = true )
    {
        ...
        */
        ...
        if( object.id.substring(0,transaction_prefix.length) == transaction_prefix ) {
            ...
            // delegate object
            if (!this._isSubbedTo("delegates", object.id)) {
                return;
            }
        ...
        if( object.id.substring(0,balance_prefix.length) == balance_prefix ) {
            ...
            if (account) {
                ...
                if (!call_orders.has(object.id)) {
                    ...
                    this.objects_by_id.set( account.get("id"), account );
                    Apis.instance().db_api().exec("get_objects", [[object.id]]); // Force subscription to the object in the delegate node by calling get_objects
                }
            }
        }
        else if( object.id.substring(0,order_prefix.length ) == order_prefix ) {
        ...
        if( missing.length ) {
            ...
            .then( vote_obj_array => {
                ...
                for( let i = 0; i < vote_obj_array.length; ++i )
                {
                    if( vote_obj_array[i] )
                    {
                        let isDelegate = vote_obj_array[i].id.substring(0, delegate_prefix.length ) == delegate_prefix;
                        this._subTo(isDelegate ? "delegates" : "committee", vote_obj_array[i].id);
...
function timeStringToDate(time_string) {
    ...
    // https://github.com/cryptonomex/evolution/issues/368

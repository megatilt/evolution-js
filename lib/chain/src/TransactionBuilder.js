...
class TransactionBuilder {
    ...
    */
    ...
    /**
        ...
        @arg {array<string>} [signer_pubkeys = null] - Optional ["EVOAbc9Def0...", ...].  These are additional signing keys.  Some balance claims require propritary address formats, the delegate node can't tell us which ones are needed so they must be passed in.  If the delegate node can figure out a signing key (mostly all other transactions), it should not be passed in here.
    ...
    */
    ...
    set_required_fees(asset_id){
        ...
        if (asset_id !== "1.3.0") {
            // This handles the fallback to paying fees in EVO if the fee pool is empty.
